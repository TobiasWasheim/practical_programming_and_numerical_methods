...
double cspline::integrate(double z) {
    int i = binsearch(x,z);
    double sum = 0;

    for (int j = 0; j < i; j++) {

        sum += y[j] * dx[j] + b[j] * dx[j] * dx[j] / 2.0 + c[j] * dx[j] * dx[j] * dx[j] / 3.0 + d[j] * dx[j] * dx[j] * dx[j] * dx[j]/4.0;
    }

    sum += y[i] * (z - x[i]) + b[i] * (z-x[i]) * (z-x[i]) / 2.0 + c[i] * (z-x[i]) * (z-x[i]) * (z-x[i]) / 3.0 + d[i] * (z-x[i]) * (z-x[i]) * (z-x[i]) * (z-x[i]);
